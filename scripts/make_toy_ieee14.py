...
def main():
    ...
    if mode == "opf_storage":
        ...
        for day_start in range(0, T, H):
            ...
            # keep the same e_init every day because terminal_equal_init=True
            # (later we can carry over day-to-day if you want)
            if day_start == 0:
        ...
        # SoC file
        # If not generated by OPF-storage mode, create a dummy SoC file
        if mode in ("pf", "opf", "both"):
            for t in range(T):
