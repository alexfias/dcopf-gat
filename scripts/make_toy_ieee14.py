...
from dcopf_gat.dcopf import DcOpfTopology, solve_dcopf_ptdf, solve_dcopf_storage_ptdf
...
def main():
    ...
    mode = "opf_storage"  # "pf" | "opf" | "both" | "opf_storage"
    ...
    elif mode == "opf_storage":
        out_dir = Path("data_ieee14_opf_storage")
    else:
    ...
    demand = rng.uniform(5, 25, size=(T, N)).astype(float)
    ...
    # ----------------------------
    # Storage (Option 1): one battery at Bus1 (index 0)
    # ----------------------------
    store_bus = 0  # Bus1
    E_max = 200.0
    P_ch_max = 50.0
    ...
    if mode == "opf_storage":
        ...
        H = 24
        assert T % H == 0, "For now, set T as multiple of 24 for daily blocks."

        pmin = np.zeros(N, dtype=float)
        c = np.linspace(10.0, 50.0, N).astype(float)

        e_init = 0.5 * E_max

        for day_start in range(0, T, H):
            d_block = demand[day_start : day_start + H, :]  # (H,N)
            pmax_block = (cf[day_start : day_start + H, :] * p_nom_gen)  # (H,N)

            pg, pinj, f, e, p_st = solve_dcopf_storage_ptdf(
                topo=topo,
                PTDF=PTDF,
                d=d_block,
                G=G,
            ...
            gen[day_start : day_start + H, :] = pg
            flows[day_start : day_start + H, :] = f
            soc[day_start : day_start + H, 0] = e[:, 0]

            if day_start == 0:
                print("=== DC-OPF+storage sanity check (day 0, t=0) ===")
                print("Balance residual:", float(pinj[0].sum()))
                print("Max line violation:", float(np.max(np.abs(f[0]) - topo.f_max)))
                print("Max |flow|:", float(np.max(np.abs(f[0]))))
                print("SoC(t=0):", float(e[0, 0]), "SoC(t=end):", float(e[-1, 0]))
                print("max |p_st| day0:", float(np.max(np.abs(p_st))))
                print("===============================================")
        ...
        # Save SoC time series (REAL, not dummy)
        stores_df = pd.DataFrame(soc, index=time_index, columns=["Store0"])
        stores_df.to_csv(out_dir / "stores_t_e.csv")

    # ----------------------------
    # Save outputs for non-storage modes
    # ----------------------------
    if mode == "pf":
    ...
    # ----------------------------
    # stores_t_e.csv (dummy only for modes without storage)
    # ----------------------------
    if mode != "opf_storage":
